...
- A recorded demo video, and any physical-device demo. The walkthrough is stills captured on the iOS simulator. Cross-device sync and the capture upload path are proven by `mobile/scripts/live-verify.ts` rather than by images; the capture UI's rendering is unproven in the walkthrough, and the README says so (dropped v1.5).
...
2. `cd mobile && pnpm tsc --noEmit && pnpm lint && pnpm test` clean. Schema mirror test asserts the Zod schema matches `schema/quote.schema.json` field-for-field. `quote-assembly-reducer` and `quote-assembly-live-fixture` assert that the assembly reducer folds real recorded pipeline events in emission order and animates only events arriving live, never replayed history — this is where the no-staged-animation invariant is mechanically proven.
3. Manual E2E on the iOS simulator, captured as the demo asset, and all of it one run against **one `quote_id`**: sign in via the emailed 6-digit code, create a painting job, seed the capture media from the committed fixtures (`backend/scripts/seed_live_demo.py seed`, because the simulator has no camera — at least 3 photos and 30 to 60 seconds of narration, which the committed fixtures satisfy), run the real pipeline (`... run <quote_id>`) and record its wall-clock, which must come in under the 90-second budget of criterion 3, watch line items assemble live with photo thumbnails attaching, receive a quote where every line cites a photo and at least one line shows the inferred flag with a vision-estimated quantity, price the unpriced line to unlock Send, send, open the share link in a desktop browser, accept, and confirm in the database that the quote status is `accepted` with exactly one `quote_accepted` event.
4. `pnpm playwright test` green on the web quote page: renders from share token, Accept persists, invalid token 404s.
5. Agent trace screen shows all seven nodes for **check 3's `quote_id`** with non-zero durations, token counts on the LLM nodes, and a second attempt group from the seeded retry. The `quote_events` rows for that same quote, queried directly, contain one `line_item_drafted` per line in emission order plus the `retry_started` row, and the line items in the screenshots match those rows one for one. Stills carry no timing, so this establishes that the screenshots depict the events the pipeline actually emitted; that those events *drive* the animation rather than being replayed over a finished quote is proven by check 2's reducer tests and check 9, not by any image.
...
7. The painter screenshot walkthrough is captured on the iOS simulator against the localhost backend and committed to `docs/screenshots/`, every still coming from **check 3's single `quote_id`** rather than stitched from several runs, covering the story arc: the stage ticker mid-generation, live assembly partway through, the dimmed retry retraction with its "Revising draft" banner, the completed quote showing photo citations and one inferred flag, the agent trace, the public web quote page, and the Accepted banner back in the app.
8. The portfolio-grade README exists: architecture, the seven-node pipeline, the hard invariants, run instructions, the embedded screenshot walkthrough, and the live web sample link. Its limitations section explicitly names: fire-and-forget generation, no payments, no offline, seeded price books, backend runs locally (not hosted), Android and a physical iPhone both running the same Expo code but neither tested on device, the demo captured on the simulator with fixture-seeded capture media so the capture UI's rendering is unproven in the walkthrough, and cross-device realtime sync proven by `live-verify.ts` rather than shown in a still. Each with its upgrade path in one sentence.
9. `cd mobile && pnpm exec tsx scripts/live-verify.ts` green against live services, with uvicorn up. This is the evidence behind criterion 6 and the substitute for the dropped two-device still: it drives the screens' real data paths and asserts realtime live assembly in emission order, subscribe-first catch-up, cross-device edit propagation under 2 seconds, RLS isolation including realtime spies, cached regenerate, and the capture upload path.

verify.sh wires checks 1 and 2 into the Stop hook so build sessions self-gate. Checks 3 through 9 are the human acceptance pass: the repo flips public after check 3, the web sample (check 6) deploys alongside, and the LinkedIn/portfolio cycle waits for checks 6 and 7 — the live sample link and the screenshot walkthrough.
